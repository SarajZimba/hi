...
#     count = 0
    ...
    context = {'dests': dests, 'feedbacks': feedbacks, 'value': value}    
...
def search(request):
    ...
    if len(query) > 80:
        ...
        blogdatasdesc = product.objects.filter(name__icontains=query)

        # blogdatas = blogdatasname.union(blogdatastag)
        # blogdatas1 = blogdatas.union(blogdatasdesc)
        paginator = Paginator(blogdatasdesc, 3)
    ...
    #  user = request.user
     ...
     if request.method == 'POST':
          ...
          if form.is_valid:
               ...
               rate.save()
                 # Assuming review has a ForeignKey to destination
               return redirect('destination_details', dest_id=dest_id)
...
def update_review(request, review_id):
     ...
     if request.method == 'POST':
          ...
          if form.is_valid():
               form.save()
               dest_id = review.destination.id  # Assuming review has a ForeignKey to destination
               return redirect('destination_details', dest_id=dest_id)
